...
@Component({
  selector: 'app-edit-consignor',
  templateUrl: './edit-consignor.component.html',
  styleUrls: ['./edit-consignor.component.css']
})
export class EditConsignorComponent implements OnInit {
  ...
  async getConsignorData() {
    this.spinner.show();
    let paramObj = {
      code: this.code
    }
    try {
      const res: any = await this.consiService.getConsignor(paramObj);

      this.consignorArr = res;

      console.log(this.consignorArr)



      this.latitude = this.consignorArr[0].metas[0].geoFence.latitude;
      this.longitude = this.consignorArr[0].metas[0].geoFence.longitude;
      this.radius = this.consignorArr[0].metas[0].geoFence.radius;

      this.createForm.patchValue({
        addressForm: this.consignorArr[0].address,
        consignorName: this.consignorArr[0].name,
        consignorCode: this.consignorArr[0].code,
        groupName: this.consignorArr[0].departmentGroup,
        consignorCoordinates: this.consignorArr[0].metas[0].geoFence.polygon,
        loadingTime: this.consignorArr[0].metas[0].loadingTime,
        geofence: this.consignorArr[0].metas[0].geoFence.type,
        radiusForm: this.consignorArr[0].metas[0].geoFence.radius
      })

      if (this.consignorArr[0].metas[0].geoFence.type == "POLYGON") {
        let path1 = this.consignorArr[0].metas[0].geoFence.polygon.split('|');
        path1.forEach((el) => {
          let cords = el.split(':');
          this.pointList.push({
        ...
        this.pointList.pop();
        console.log("path", this.pointList)
      }
    }
    catch (err) {
      console.log(err)
